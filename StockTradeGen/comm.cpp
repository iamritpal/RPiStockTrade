...
void Comm::addTxByte(unsigned char byte)
{
	txbuffer[txIx++] = byte;
}
...
void Comm::addRxByte(unsigned char byte)
{
	rxbuffer[rxIx++] = byte;
}
...
void Comm::transmit(void)
{
	if (uart0_filestream != -1)
	{
		int count = write(uart0_filestream, &txbuffer[0], txIx);		//Filestream, bytes to write, number of bytes to write
		if (count < 0)
		{
